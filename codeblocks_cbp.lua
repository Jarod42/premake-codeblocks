	...
	function m.configurations(prj)
		...
		for _,platform in pairs(platforms) do
			for _,cfg in pairs(platform.configs) do
				...
				_p(4,'<Compiler>')
				p.escaper(p.quote)
				local flags = table.join(compiler.getcflags(cfg), compiler.getcxxflags(cfg), compiler.getdefines(cfg.defines, cfg), compiler.getundefines(cfg.undefines), cfg.buildoptions)
